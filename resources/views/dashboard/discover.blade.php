...
<title>Discover Marketplace - FarmDirect</title>
...
<header class="mb-12">
    <h1 class="text-5xl font-black text-on-surface tracking-tighter mb-4">Discover Marketplace</h1>
    <p class="text-on-surface-variant font-medium max-w-2xl">Browse the finest harvests across India. Filter by category, region, or organic certification.</p>
</header>

<!-- Filter Chips -->
<div class="flex flex-wrap gap-3 mb-10">
    <a href="{{ route('buyer.discover') }}" class="px-6 py-2.5 {{ !request('category') ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-white border border-outline-variant/30 text-on-surface-variant hover:bg-surface-container' }} font-black text-[10px] rounded-full uppercase tracking-widest transition-all">All Crops</a>
    
    @foreach(['Grains', 'Fruits', 'Vegetables', 'Spices'] as $cat)
        <a href="{{ route('buyer.discover', ['category' => $cat]) }}" class="px-6 py-2.5 {{ request('category') == $cat ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-white border border-outline-variant/30 text-on-surface-variant hover:bg-surface-container' }} font-black text-[10px] rounded-full uppercase tracking-widest transition-all">{{ $cat }}</a>
    @endforeach
</div>
    ...
    @forelse($crops ?? [] as $crop)
        <div class="group bg-white rounded-3xl overflow-hidden border border-outline-variant/30 shadow-soft hover:shadow-premium transition-all duration-500 hover:-translate-y-2">
            <div class="relative h-64 overflow-hidden">
                ...
                <div class="absolute bottom-4 right-4 bg-primary text-white px-4 py-1.5 rounded-xl font-black text-sm shadow-xl">
                    ₹{{ $crop->price_per_unit }}<span class="text-[10px] font-medium opacity-80">/{{ $crop->unit }}</span>
                </div>
            </div>
            <div class="p-6">
                ...
                <div class="flex flex-col gap-3">
                    <form action="{{ route('cart.add') }}" method="POST" class="flex flex-col gap-3">
                        ...
                        <div class="flex items-center gap-2 bg-surface-container-low p-2 rounded-xl border border-outline-variant/10">
                            <span class="text-[10px] font-black text-on-surface-variant/40 uppercase tracking-widest pl-2">QTY (kg)</span>
                            <input type="number" name="quantity" value="15" min="15" class="w-full bg-transparent border-none text-right font-black text-primary focus:ring-0" required>
                        </div>
                        <button type="submit" class="w-full bg-primary text-white py-3 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-primary/90 transition-all shadow-lg shadow-primary/10">Add to Cart</button>
                    </form>
                    <button type="button" onclick="openBidModal('{{ $crop->id }}', '{{ addslashes($crop->name) }}', '{{ $crop->price_per_unit }}', '{{ $crop->unit }}')" class="w-full bg-surface-container text-on-surface py-3 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-surface-container-high transition-all">Negotiate / Bid</button>
                </div>
            </div>
        </div>
    @empty
        <div class="col-span-3 py-20 text-center bg-white rounded-[3rem] border border-outline-variant/30">
            <span class="material-symbols-outlined text-6xl text-stone-200 mb-4">agriculture</span>
            <h3 class="text-2xl font-black text-on-surface">Market is empty right now</h3>
            <p class="text-on-surface-variant">Check back later for fresh listings.</p>
        </div>
    @endforelse
</div>
</section>
...
<!-- Place Bid Modal -->
<div id="bid-modal" class="fixed inset-0 bg-black/50 z-[100] hidden flex items-center justify-center p-4 backdrop-blur-sm transition-opacity duration-300">
    <div class="bg-white rounded-[2.5rem] p-8 w-full max-w-md shadow-premium transform transition-all duration-300 scale-95 opacity-0" id="bid-modal-content">
        <div class="flex justify-between items-center mb-6">
            <h3 class="text-xl font-black text-on-surface">Place a Bid</h3>
            <button onclick="closeBidModal()" class="w-10 h-10 flex items-center justify-center hover:bg-surface-container rounded-xl transition-all">
                <span class="material-symbols-outlined text-sm">close</span>
            </button>
        </div>
        
        <div class="mb-6 p-4 bg-primary/5 rounded-2xl border border-primary/10">
            <p class="text-[10px] font-bold text-primary uppercase tracking-widest mb-1">Target Crop</p>
            <p id="bid-crop-name" class="font-black text-lg text-on-surface">Crop Name</p>
            <p id="bid-crop-price" class="text-sm font-medium text-on-surface-variant mt-1">Asking: ₹0/unit</p>
        </div>

        <form action="{{ route('bids.place') }}" method="POST" class="space-y-4">
            @csrf
            <input type="hidden" name="crop_id" id="bid-crop-id">
            
            <div>
                <label class="block text-[10px] font-black text-on-surface-variant/40 uppercase tracking-widest mb-1">Your Bid Amount (₹)</label>
                <input type="number" name="amount" id="bid-amount" class="w-full bg-surface-container border-none rounded-xl px-4 py-3 font-bold text-lg focus:ring-2 focus:ring-primary/20 transition-all" placeholder="Enter bid amount" required min="1">
                <p class="text-xs text-stone-400 mt-2">Enter the total amount you are willing to pay.</p>
            </div>
            
            <button type="submit" class="w-full bg-primary text-white py-4 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-primary/90 transition-all shadow-lg shadow-primary/20 mt-4">Submit Bid</button>
        </form>
    </div>
</div>
...
<script>
    ...
    function openBidModal(cropId, cropName, askingPrice, unit) {
        document.getElementById('bid-crop-id').value = cropId;
        document.getElementById('bid-crop-name').textContent = cropName;
        document.getElementById('bid-crop-price').textContent = `Asking: ₹${askingPrice}/${unit}`;
